    ...
    while counter <= 2000: 
        ...
        for i in successors:
            if i not in seen_states:
                ...
                #if the proposed sequence has less distance than the current then adopt it
...
if __name__ == '__main__':
    ...
    # Calculating the least dist using steepest descent hill climbing(5 Iterations, same initial sequence)
    for i in range(5):
        least_distance, current_seq, cost = hill_climb_steepest_descent(random_seq,
                                                            coordinates)
    ...
    #writing to file
    cwd = os.getcwd()
    #output_path =  cwd + "/results/Hill_climbing_steepest_descent_49_cities.txt"
    output_path =  cwd + "/results/Hill_climbing_steepest_descent_cities_full.txt"
    L = ["cost every 2000 iteration: " + str(cost_every_2000), 
    "\n\nLeast distance from Steepest descent: "+ str(int(best_cost_ever)),
    ...
    print("Least distance from Steepest descent:", best_cost_ever)
